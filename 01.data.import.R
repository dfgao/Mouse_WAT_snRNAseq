...
soupx_dbl <- function(sp){
  ...
  sceList_clean <- lapply(samples,function(pro){
    print(pro)
    sce_raw <- Read10X(file.path(dir,pro,'outs/raw'))
    
    sce_filtered <- Read10X(file.path(dir,pro,'outs/filtered'))
...
for (samples in c('Clean_qc')) {
  ...
  tmp.mer$condition[which(str_detect(tmp.mer$cells, "^HP"))] <- "HP"
  tmp.mer$condition[which(str_detect(tmp.mer$cells, "^LP"))] <- "LP"
  assign(tmp,tmp.mer)
...
p2 <- Clean_qc.merge@meta.data %>% 
  ...
  theme_classic() +
  ylab("Cell density") +
  geom_vline(xintercept = c(350,600))
p1 + p2
...
Clean_qc.merge@meta.data %>% 
  ...
  theme_classic() +
  scale_x_log10() + 
  geom_vline(xintercept = c(950,650) )
...
Clean_qc.merge.filtered <- subset(Clean_qc.merge, 
                                  cells = unique(c(rownames(Clean_qc.merge@meta.data[Clean_qc.merge$condition == 'HP' & 
                                                                                       Clean_qc.merge$nCount_RNA > 950 & 
                                                                                       Clean_qc.merge$nFeature_RNA > 600 &
                                                                                       Clean_qc.merge$mitoRatio < mito.mad.upper,] ),
                                                   rownames(Clean_qc.merge@meta.data[Clean_qc.merge$sample == 'LP1' &
                                                                                       Clean_qc.merge$nCount_RNA > 950 &
                                                                                       Clean_qc.merge$nFeature_RNA > 600 &
                                                                                       Clean_qc.merge$mitoRatio < mito.mad.upper,] ),
                                                   rownames(Clean_qc.merge@meta.data[Clean_qc.merge$sample == 'LP2' &
                                                                                       Clean_qc.merge$nFeature_RNA > 350 &
                                                                                       Clean_qc.merge$nCount_RNA > 650 &
                                                                                       Clean_qc.merge$mitoRatio < mito.mad.upper,]))
                                  ))
